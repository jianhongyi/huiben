<template>
    <div class="reset-wrapper" :style="{ height: height + 'px' }">
        <div class="title"><span>重置“外研通行证”账号密码</span></div>
        <div class="cell">
        ...
        <div class="cell">
            ...
            <div class="value">
                <input v-model="verifiyCode" type="tel" name="verifycode" placeholder="请输入验证码" class="control" />
            </div>
        ...
        <div class="cell">
            ...
            <div
                class="icon"
                :class="pwdInputSecType === 'password' ? 'hidden' : 'display'"
                @click="handleChangePwdSecType"
            ></div>
        </div>
        <div class="info">密码必须由英文字母及数字组成，长度为6-12位</div>
        <div class="change-btn" :class="{ disabled: btnDisabled }" @click="handleSubmit">更改密码</div>
    </div>
</template>

<script>
import ForgetPassword from "@/apis/forgetPassword/index";
import CaptchaForgetPassword from "@/apis/captcha/forgetPassword/index";
export default {
    data() {
        return {
            phone: "",
    ...
    computed: {
        btnDisabled() {
            return !(this.phoneValid && this.password.length && this.passwordSec.length && this.verifiyCode.length);
        },
    },
    mounted() {
    ...
    methods: {
        async handleSubmit() {
            const { phone, password, passwordSec, verifiyCode, btnDisabled } = this;
            if (btnDisabled) return;
            if (password !== passwordSec) {
                this.$toast({ content: "两次密码输入不一致" });
                return;
            }
            const { success, info } = await ForgetPassword(phone, password, verifiyCode);
            if (!success) {
        ...
        async handleGetVerifyCode() {
            ...
            const { success, errorCode, info } = await CaptchaForgetPassword(phone);
            if (errorCode) {
                if (errorCode === 302) {
                    this.$dialog({
        ...
        handleChangePwdSecType() {
            const { pwdInputSecType } = this;
            this.pwdInputSecType = pwdInputSecType === "password" ? "text" : "password";
        },
    },
};
</script>
